...
class Transformer_UNET(nn.Module):
    def __init__(self, img_shape=(64, 64, 64), input_dim=1, output_dim=4, embed_dim=768, patch_size=16,
...
def infer_classifier(density_map_splits_dir, input_data_dir, density_map_name, atom_checkpoint, infer_run_on, infer_on_gpu):
    ...
    predicts = trainer.predict(model, dataloaders=test_loader, ckpt_path=atom_checkpoint)
...
if __name__ == "__main__":
    ...
    infer_classifier(density_map_splits_dir=density_map_splits_dir, input_data_dir=input_data_dir, density_map_name=density_map,
                          atom_checkpoint=atom_checkpoint, infer_run_on=infer_run_on, infer_on_gpu=infer_run_gpu)